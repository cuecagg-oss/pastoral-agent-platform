...
No caminho THÁNOS, o roteador persiste a mensagem de usuário uma vez e a fachada persiste uma resposta de assistente. Uma exceção não tratada no núcleo delega ao legado com `persistUserMessage=false`. Por isso, o fallback não cria uma segunda mensagem de usuário. O fallback determinístico do orquestrador mantém a resposta composta e é identificado sem causa bruta.

O evento `thanos.route` contém apenas rota, decisão categórica, versão sanitizada, modo, quantidade/lista de ferramentas, presença de fallback e duração. As etapas continuam a registrar seus próprios eventos. Nenhuma telemetria contém pergunta, evidência, allowlist, segredo, token, URL interna ou stack trace.
...
Foram aprovados testes determinísticos para configuração padrão, audience por tenant/usuário, intenções fechadas, planos de uma, duas e três ferramentas, isolamento do contexto, auditoria por etapa, telemetria sanitizada, kill switch, fallback determinístico e fallback legado sem duplicação. A regressão completa aprovou 102 testes em 34 arquivos; `pnpm check`, build e auditoria de dependências de produção também concluíram sem falhas.
