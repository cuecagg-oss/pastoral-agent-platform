...
| Orquestração central de leitura | Aprovado | `ThanosReadOrchestrator` recebe somente portas genéricas, exige capability antes da ferramenta, preserva `requestId` e audita decisões sem incluir prompt ou evidência. |
| Fachada Pastoral | Aprovado | `PastoralThanosFacade` adapta o núcleo ao contrato `AgentResponse`, executa a ferramenta uma única vez e permanece desacoplada do caminho público atual para rollback por delegação. |
| Skill Pastoral de leitura | Aprovado | A skill `pastoral-assistant` aceita somente o canal `chat`, exige `agent:read` e lista apenas as ferramentas categorizadas como `READ`; ferramentas `SENSITIVE` e `WRITE` não integram o piloto. |
| Adaptador de ferramentas | Aprovado | O adaptador do workspace seleciona somente ferramenta permitida pela skill e pelo catálogo já resolvido por papel e organização, delegando a execução ao domínio Pastoral sem reimplementar autorização no núcleo. |
| Negação declarativa | Aprovado | Os testes cobrem canal, capability e catálogo incompatíveis, enquanto o registro fechado recusa workspace/skill desconhecidos e o contexto deriva o tenant exclusivamente da sessão server-side. |
| Piloto multi-step READ | Aprovado | `ThanosMultiStepReadOrchestrator` aceita somente dois ou três passos READ, preserva o mesmo `ThanosContext` em toda a sequência, acumula evidências compostas e realiza uma única geração final. |
