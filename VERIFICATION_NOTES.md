...
O piloto multi-step encadeia `consultar_celulas` e `consultar_presenca` com o mesmo contexto de organização, compõe apenas evidências autorizadas, audita cada etapa e usa fallback determinístico para falha operacional. A validação final aprovou `pnpm check`, **94 testes Vitest em 32 arquivos**, build de produção e auditoria de dependências de produção sem vulnerabilidades conhecidas. A adoção do núcleo pelo Gateway público ficou registrada como decisão futura, mantendo texto, voz, histórico e confirmações de escrita no caminho compatível atual.
